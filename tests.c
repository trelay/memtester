...
int compare_regions(ulv *bufa, ulv *bufb, size_t count, FILE *f) {
    ...
    for (i = 0; i < count; i++, p1++, p2++) {
        if (*p1 != *p2) {
            ...
            /* fprintf(f,"Skipping to next test..."); */
            r = -1;
        }
    }
    return r;
...
int test_stuck_address(ulv *bufa, size_t count, FILE *f) {
    ...
    fflush(f);
    for (j = 0; j < 16; j++) {
        fprintf(f,"\b\b\b\b\b\b\b\b\b\b\b");
        p1 = (ulv *) bufa;
        fprintf(f,"setting %3u", j);
        fflush(f);
        for (i = 0; i < count; i++) {
    ...
    fprintf(f,"\b\b\b\b\b\b\b\b\b\b\b           \b\b\b\b\b\b\b\b\b\b\b");
    fflush(f);
    return 0;
}
...
int test_random_value(ulv *bufa, ulv *bufb, size_t count, FILE *f) {
    ...
    fprintf(f,"\b \b");
    fflush(f);
    return compare_regions(bufa, bufb, count,f);
}
...
int test_solidbits_comparison(ulv *bufa, ulv *bufb, size_t count, FILE *f) {
    ...
    for (j = 0; j < 64; j++) {
        ...
        fprintf(f,"testing %3u", j);
        fflush(f);
        if (compare_regions(bufa, bufb, count,f)) {
            return -1;
        }
    }
    fprintf(f,"\b\b\b\b\b\b\b\b\b\b\b           \b\b\b\b\b\b\b\b\b\b\b");
